...
class MayaQWidgetDockableMixin(MayaQWidgetBaseMixin):
    ...
    def setDockableParameters(self, dockable=None, floating=None, area=None, allowedArea=None, width=None, height=None, x=None, y=None, *args, **kwargs):
        ...
        if (dockable == True) or (dockable == None and self.isDockable()): # == Handle docked window ==
            ...
            if allowedArea != None:
                allowed_areas = None
                area_items = allowedArea.split()
                if 'all' in area_items:
                    allowed_areas = Qt.AllDockWidgetAreas
                    dockWidget.setAllowedAreas(allowed_areas)
                else:
                    for area_val in area_items:
                        area_val = dockAreaStrMap.get(area_val, None)
                        if not area_val is None:
                            if allowed_areas is None:
                                allowed_areas = area_val
                            else:
                                allowed_areas = allowed_areas | area_val
                    if allowed_areas is None:
                        allowed_areas = Qt.AllDockWidgetAreas
                    dockWidget.setAllowedAreas(allowed_areas)
